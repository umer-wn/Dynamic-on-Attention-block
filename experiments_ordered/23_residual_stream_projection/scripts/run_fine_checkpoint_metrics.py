...
TOKEN_MANIFEST = REPO / "experiments_ordered/16_frequency_stratified_window_jacobian/manifests/frequency_stratified_tokens.csv"
CACHE = Path("/home/luohaoming/model_feature_cache/hf_cache")
STATE_ROOT = Path("/data1/luohaoming/model_feature_experiments/experiment23_residual_stream_projection")
...
def refresh(toks: list[dict]):
    parts=ROOT/"processed/fine_metric_parts"; rows=[]
    for cp in CHECKPOINTS:
        path=parts/f"{cp}.csv"
        if path.exists(): rows.extend(read(path))
    if not rows:return
    write(ROOT/"processed/fine_checkpoint_metrics_raw.csv",rows)
    ...
    write(ROOT/"processed/fine_checkpoint_metrics.csv",out)


def main():
    p=argparse.ArgumentParser();p.add_argument("--device",default="cuda:2");p.add_argument("--chunk",type=int,default=16);args=p.parse_args();device=torch.device(args.device);toks=tokens();parts=ROOT/"processed/fine_metric_parts";parts.mkdir(parents=True,exist_ok=True)
    ...
    for cp in CHECKPOINTS:
        ...
        del model,trajectory;torch.cuda.empty_cache();refresh(toks)
    refresh(toks)
