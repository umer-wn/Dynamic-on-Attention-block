...
def main():
    p=argparse.ArgumentParser();p.add_argument("--device",default="cuda:1");p.add_argument("--chunk",type=int,default=128);args=p.parse_args();device=torch.device(args.device);toks=tokens();parts=ROOT/"processed/fine_metric_parts_8token";parts.mkdir(parents=True,exist_ok=True)
